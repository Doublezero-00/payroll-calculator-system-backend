...
export async function updateSalary(req, res) {
  try {
    ...
    if(user.length === 0) {
      ...
      await db.query("UPDATE salaries SET base_salary = ?, allowance = ?, deductions = ?, overtime_hours = ?, overtime_rate = ?, net_salary = ? WHERE id = ?", [base_salary, allowance, deductions, overtime_hours, overtime_rate, net_salary, id]);
